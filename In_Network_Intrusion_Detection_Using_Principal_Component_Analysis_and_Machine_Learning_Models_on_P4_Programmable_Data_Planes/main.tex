...
\maketitle
\begin{abstract}
IoT gateways are a natural point for attack detection across heterogeneous, resource-constrained devices, but conventional intrusion detection that mirrors traffic to an external security analyzer adds latency and bandwidth overhead that line-rate IoT deployments cannot tolerate. P4 programmable switches address this gap: tree-based classifiers already run directly in the data plane at line rate, but Principal Component Analysis (PCA) requires multiply-accumulate operations that match-action pipelines cannot natively express, so PCA projection has so far been confined to the control plane, computed offline before the data-plane classifier runs. This paper presents the first P4 intrusion-detection pipeline that brings both PCA projection and tree classification directly into the data plane under a single range-match abstraction, and evaluates the design on a Raspberry Pi~4 running BMv2 (\texttt{simple\_switch\_grpc}). Across $32$ deployed configurations (Decision Tree and 4-tree Random Forest, with and without PCA), the raw range-match classifier achieves the highest in-distribution macro F1 on CIC-IoT 2023 ($98.47\%$) but degrades to $15.57\%$ on a disjoint Mu-IoT capture because port associations learned on the source dataset do not generalize. The recommended PCA $+$ Decision Tree at $k{=}7$ achieves $95.79\%$ in distribution and $48.31\%$ on the disjoint capture, a $32.74$-point cross-dataset gain for only $2.68$ points of in-distribution cost, making strong in-distribution accuracy and cross-dataset robustness jointly attainable. The recommended operating point clears the Tofino-1 per-stage match-key budget; stage-count compaction is the path to full deployment.
