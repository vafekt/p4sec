...
\maketitle
\begin{abstract}
Tree-based classifiers for IoT intrusion detection already run inside the P4 data plane, but Principal Component Analysis (PCA) needs multiply-accumulate operations unavailable in match-action pipelines, so PCA-projected features are typically computed off-switch before classification. This paper presents the first P4 intrusion-detection pipeline that performs both the PCA projection and the downstream tree classification under a single range-match abstraction, and evaluates the design on a Raspberry Pi~4 running BMv2 (\texttt{simple\_switch\_grpc}) with the v1model architecture. Across $32$ deployed configurations covering a Decision Tree and a 4-tree Random Forest with and without PCA, the raw range-match classifier achieves the highest in-distribution macro F1 on CIC-IoT 2023 ($98.47\%$), yet degrades sharply on a disjoint Mu-IoT capture ($15.57\%$ macro F1) because dataset-specific port associations learned on the source dataset do not generalize. The recommended PCA $+$ Decision Tree at $k{=}7$ achieves $95.79\%$ macro F1 in distribution and $48.31\%$ on the disjoint capture, a $32.74$ percentage-point cross-dataset gain at the cost of only $2.68$ percentage points in distribution, showing that projecting raw features into composite components breaks the classifier's dependence on any individual feature and makes strong in-distribution accuracy and cross-dataset robustness jointly attainable. The recommended operating point runs on the Raspberry Pi~4 testbed and clears the Tofino-1 per-stage match-key budget; stage-count compaction via feature selection, shared register placement, and compound PCA tables is the identified path to full Tofino-1 deployment.
