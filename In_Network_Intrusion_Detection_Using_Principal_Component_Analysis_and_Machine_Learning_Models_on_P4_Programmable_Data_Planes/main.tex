...
\maketitle
\begin{abstract}
IoT gateways are critical vantage points for detecting attacks across heterogeneous, resource-constrained devices, but conventional intrusion detection that mirrors traffic to an external classifier adds latency and bandwidth overhead that line-rate IoT deployments cannot tolerate. P4 programmable switches address this gap: tree-based classifiers already run directly in the data plane at line rate, but Principal Component Analysis (PCA) needs multiply-accumulate operations unavailable in match-action pipelines, so PCA-projected features are typically computed off-switch before classification. This paper presents the first P4 intrusion-detection pipeline that performs PCA projection and tree classification under a single range-match abstraction, and evaluates the design on a Raspberry Pi~4 running BMv2 (\texttt{simple\_switch\_grpc}). Across $32$ deployed configurations covering a Decision Tree and a 4-tree Random Forest with and without PCA, the raw range-match classifier achieves the highest in-distribution macro F1 on CIC-IoT 2023 ($98.47\%$), yet degrades sharply on a disjoint Mu-IoT capture ($15.57\%$ macro F1) because port associations learned on the source dataset do not generalize. The recommended PCA $+$ Decision Tree at $k{=}7$ achieves $95.79\%$ macro F1 in distribution and $48.31\%$ on the disjoint capture, a $32.74$ percentage-point cross-dataset gain at the cost of only $2.68$ percentage points in distribution, making strong in-distribution accuracy and cross-dataset robustness jointly attainable. The recommended operating point clears the Tofino-1 per-stage match-key budget; stage-count compaction is the identified path to full Tofino-1 deployment.
