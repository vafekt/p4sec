...
Outputs (same filenames / same contract as every other step 2):
    ...
    tables/reduction_config.json  method = "pca_linear"
...
def parse_args():
    p = argparse.ArgumentParser(
        description="PCA-linear: additive per-feature projection (no surrogate)")
    p.add_argument("--components", "-k", type=int, default=7,
                   help="Number of PCA components (default: 7)")
    p.add_argument("--bits", "-b", type=int, default=32,
                   help="Quantization bits for the PCA codes (default: 32)")
    return p.parse_args()
...
def main():
    ...
    # ── Fit StandardScaler + PCA on the QUANTIZED features ───────────────────
    scaler = StandardScaler()
    Z      = scaler.fit_transform(X_q.astype(float))
    pca    = PCA(n_components=k, random_state=42)
    PC     = pca.fit_transform(Z)                    # (N, k)
    pc_min = PC.min(axis=0)
    pc_max = PC.max(axis=0)
    pc_rng = np.where((pc_max - pc_min) == 0, 1.0, pc_max - pc_min)

    W      = pca.components_                          # (k, 18) in scaled space
    pcamn  = pca.mean_                                # (18,)  ~0 after scaling
    ...
    for j in range(k):
        print(f"  PC{j+1}_code range: {PC_code[:, j].min()} -> {PC_code[:, j].max()}")
    ...
    for i, feat in enumerate(P4_FEATURE_COLS):
        ...
        for idx, v in enumerate(vals):
            ...
            rules.append(f"{feat:16s} in [{lo:>6},{hi:>6}] (rep={v:>6}) -> "
                         + " ".join(f"PC{j+1}+={d[j]}" for j in range(k)) + "\n")
            total_entries += 1
    ...
    mp = {f"PC{j+1}_float": PC[:, j] for j in range(k)}
    mp.update({f"PC{j+1}_code": PC_code[:, j] for j in range(k)})
    mp["Label"] = y
