...
# ==========================================================
# 13. Generate DT classification commands (PCA components -> traffic class)
#     Maps 2 PCA component codes to traffic classification
# ==========================================================
# Train a new DecisionTreeClassifier on PCA codes for classification
clf_dt = DecisionTreeClassifier(random_state=42)
clf_dt.fit(PC_code_train, y_train)

# Extract decision tree rules for P4 DT table
def write_dt_p4_commands(clf, max_val, filename):
    """
    Generate P4 table_add commands for DT classification table (ml_code).
    Maps (PC1_code, PC2_code) ranges to traffic class.
    Components numbered from 1 (PC1, PC2, etc.).
    Uses dynamic max_val based on BITS parameter.
    """
    tree_ = clf.tree_
    left = tree_.children_left
    right = tree_.children_right
    ...
    class_indices = np.argmax(clf.tree_.value, axis=2).flatten()
    classes = clf.classes_
    
    priority = [0]
    
    def extract_leaf_rules(node_id, path):
        is_leaf = (left[node_id] == right[node_id])
        ...
        if is_leaf:
            ...
            new_path = []
            for (n_id, sign) in path:
                feat_idx = feature_indices[n_id]
                if feat_idx == -2:
                    continue
                thr = threshold[n_id]
                new_path.append((feat_idx, sign, thr))
            
            # Determine feature ranges for PC1 and PC2
            pc1_lo, pc1_hi = None, None
            pc2_lo, pc2_hi = None, None
            
            for (feat_idx, sign, thr) in new_path:
                if feat_idx == 0:  # PC1_code
                    if sign == "<=":
                        pc1_hi = thr
                    else:
                        pc1_lo = thr
                elif feat_idx == 1:  # PC2_code
                    if sign == "<=":
                        pc2_hi = thr
                    else:
                        pc2_lo = thr
            
            # Defaults for unconstrained ranges
            if pc1_lo is None:
                pc1_lo = -1
            if pc1_hi is None:
                pc1_hi = max_val  # use dynamic max value
            if pc2_lo is None:
                pc2_lo = -1
            if pc2_hi is None:
                pc2_hi = max_val  # use dynamic max value
            
            # Convert tree bounds to range match bounds
            pc1_lo = int(pc1_lo) + 1
            pc1_hi = int(pc1_hi)
            pc2_lo = int(pc2_lo) + 1
            pc2_hi = int(pc2_hi)
            ...
            # Get predicted class
            class_idx = class_indices[node_id]
            class_val = classes[class_idx]
            
            # Map class name to numeric value (0-indexed in label list)
            class_labels = sorted(np.unique(y))
            class_num = class_labels.index(class_val) if class_val in class_labels else 0
            
            f.write(f"table_add MyIngress.ml_code set_result {pc1_lo}->{pc1_hi} {pc2_lo}->{pc2_hi} => {class_num} {priority[0]}\n")
            return
        
        # Left (<=)
        new_path_left = path.copy()
        new_path_left.append((node_id, "<="))
        extract_leaf_rules(left[node_id], new_path_left)
        
        # Right (>)
        new_path_right = path.copy()
        new_path_right.append((node_id, ">"))
        extract_leaf_rules(right[node_id], new_path_right)
    
    with open(filename, "w") as f:
        f.write("# DT Classification Rules: (PC1_code, PC2_code) -> traffic class\n\n")
        extract_leaf_rules(0, [])

dt_commands_path = os.path.join(TABLES_DIR, "dt_commands.txt")
write_dt_p4_commands(clf_dt, MAX_VAL, dt_commands_path)
print(f"P4 table_add commands for DT classification saved to: {dt_commands_path}")

# Also write IF rules for DT classification
def write_dt_if_rules(clf, filename):
    """Write human-readable DT classification rules."""
    tree_ = clf.tree_
    left = tree_.children_left
    right = tree_.children_right
    threshold = tree_.threshold
    ...
    def dfs(node_id, path):
        ...
        if is_leaf:
            ...
            for (n_id, sign) in path:
                ...
                if feat_idx == -2:
                    continue
                pc_name = f"PC{feat_idx + 1}_code"
                thr = threshold[n_id]
                clauses.append(f"{pc_name} {sign} {int(thr)}")
            ...
            rule = f"IF ({condition_str}) THEN class = {class_val};\n"
            f.write(rule)
            return
    ...
    with open(filename, "w") as f:
        f.write("# DT Classification Rules (IF-THEN)\n")
        f.write("# Maps (PC1_code, PC2_code) to traffic class\n\n")
        dfs(0, [])

dt_if_path = os.path.join(TABLES_DIR, "dt_if_rules.txt")
write_dt_if_rules(clf_dt, dt_if_path)
print(f"IF rules (PC codes -> traffic class) saved to: {dt_if_path}")
